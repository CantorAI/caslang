...
namespace CasLang
{
    ...
    {
        std::mutex m_lock;
        std::unique_lock<std::mutex>* m_activeLock = nullptr;
    ...
    public:
        ...
        inline void operator<<(std::mutex* l)
        {
            if (m_level <= m_dumpLevel)
            {
                m_buffer << '\n';
        ...
        inline std::mutex* LineEnd()
        {
            return &m_lock;
        }

        inline std::mutex* End()
        {
            return &m_lock;
        }
        ...
        {
            if (!m_activeLock) {
                m_activeLock = new std::unique_lock<std::mutex>(m_lock);
            }
        }
